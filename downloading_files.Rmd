...
```{r}
df <- read.table( "resources/output.csv" , sep = ",",header= TRUE )
head(df)
```
...
```{r}
df <- read.csv( "resources/output.csv")
head(df)
```
...
```{r}
chicago <- readRDS("resources/chicago.rds")
dim(chicago)
str(chicago)
```
...
```{r}
db <- DBI::dbConnect(RSQLite::SQLite(), "resources/db_sql.sqlite")
```
