...
##### 1.check unique of the values
##### 2.check null values, consider to drop values or adding a missing indicator
##### 3.normalize numerical features
##### 4.handle categorical feature using Label encoding or OneHotencoding or other...
...
#### 1.Histograms: R base code:hist(x, …)
##### 1.different bin boundaries different graphs.
##### 2.shape: 
               skew to left means mean < median < mode
               skew to right means mean > median > mode

#### 2.Boxplot: R base code: boxplot(x, data=),       
##### 1.multiple boxplots usually reorder by median from high to low
##### 2.will show outliers on the graph. outliers are 
...
##### 1. Sort in logical order of the categories (level1, level2, lever3..)(Ordinal data)
##### 2. Sort from highest to lowest count (Nominal data)
<br/>

#### Cleveland dot plot 

#### R codes for factor data
##### 1.don't use" "levels(x) =  ", only use "level(x)"
##### 2.use "fct_reorder()" to assign new factor levels
##### 3.use "fct_inorder()" to set level order to row order
##### 4.use "fct_relevel()" to move levels to beginning to change the level order
##### 5.use "fct_infreq()" to order the levels by decreasing frequency
##### 6.use "fct_rev()" to reverse the order of factor levels 
##### 7.use "fct_explicit_na()" to turn NAs into a real factor level
<br/>
